...
controllers.controller('TemplatesController', ['$routeParams', '$scope',  'TemplatesService',
	...
	unbindFormWatch = $scope.$watch (()-> vt.tempForm), ((newVal, oldVal)->
		if vt.tempForm
			...
			$scope.$on('$locationChangeStart', (event)->
				vt.template.id && !vt.tempForm.$pristine && !confirm('There are unsaved changes. Press cancel to stop navigation.') && event.preventDefault()
			)
			unbindFormWatch()
	)
